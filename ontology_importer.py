...
class OntologyImporterPlugin(AbstractPlugin):
    ...
    def importAnnotations(self, diagram):
        ...
        for ann in annotations:
            ...
            annotation = AnnotationAssertion(iri, property, value, language=lang)
            ...
            for ann in ontoAnno:
                ...
                self.session.undostack.push(
                        CommandIRIAddAnnotationAssertion(self.project, self.project.ontologyIRI,
                                                         annotation))
        ...
        if dialog.exec_():
            ...
            if diagram:
                ...
                with BusyProgressDialog('Loading Ontology', 0.5):
                    ...
                    try:
                        ...
                        for x in dialog.selectedFiles():
                            ...
                            # DROP ALL TABLES: remove this part #
                            '''
                            cursor.execute("DROP TABLE ontology")
                            cursor.execute("DROP TABLE project")
                            cursor.execute("DROP TABLE importation")
                            cursor.execute("DROP TABLE axiom")
                            cursor.execute("DROP TABLE drawn")
                            # Check db tables #
                            cursor.execute("SELECT name FROM sqlite_master WHERE type='table';")

                            print('tables:', cursor.fetchall())
                            '''
                            # create schema: remove 'not' #
                            if db_is_new:
                                print('Creazione dello schema vero')
                            ...
                            '''
                            cursor = conn.cursor()
                            cursor.execute("SELECT * FROM importation")
                            print('importation before importation added:', cursor.fetchall())

                            cursor = conn.cursor()
                            cursor.execute('SELECT * FROM drawn')
                            print('drawn before importation added:', cursor.fetchall())
                            '''
                ...
                for ax in self.axioms:
                    ...
                    if isinstance(ax, self.OWLAnnotationAssertionAxiom) or (isinstance(ax, self.OWLSubClassOfAxiom) and not ax.getSuperClass().isAnonymous() and not ax.getSubClass().isAnonymous()) or (isinstance(ax, self.OWLDeclarationAxiom) and ax.getEntity().isType(self.EntityType.CLASS)) or (isinstance(ax, self.OWLDeclarationAxiom) and ax.getEntity().isType(self.EntityType.ANNOTATION_PROPERTY)) or (isinstance(ax, self.OWLSubAnnotationPropertyOfAxiom)):
                    ### INSERT DRAWN AXIOMS IN Drawn Table ###
                        ...
                        conn.execute("""
                                    ...
                                    values (?, ?, ?, ?, ?, ?)
                                    """, (str(self.project.ontologyIRI), self.project.version, self.ontology_iri, self.ontology_version, str(axiom), str(self.session)))
                        conn.commit()
                        processed.append(ax)
                ...
                print(len(processed)+len(not_processed) == len(total))
                '''
                '''
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM importation')
                print('importation after importation added:', cursor.fetchall())

                cursor = conn.cursor()
                cursor.execute('SELECT * FROM drawn')
                print('drawn after importation added:', cursor.fetchall())
                '''
    ...
    def dispose(self):
        ...
        if self.session.saved:
            ...
            with conn:
                ...
                conn.execute('delete from importation where project_iri = ? and project_version = ? and session_id = ?', (str(self.project.ontologyIRI), self.project.version, str(self.session)))
                conn.commit()
                ### REMOVE DRAWS NOT SAVED ###
                conn.execute(
                    'delete from drawn where project_iri = ? and project_version = ? and session_id = ?',
                    (str(self.project.ontologyIRI), self.project.version, str(self.session)))
                conn.commit()
...
class AxiomsWindow(QtWidgets.QDialog):
    ...
    def __init__(self, not_drawn, project):
        ...
        for k in not_drawn.keys():
            ...
            a.setFont(QFont('AnyStyle', 8, QFont.DemiBold))
            layout1.addWidget(a)
            for ax in not_drawn[k]:

                check = QCheckBox(str(ax))
